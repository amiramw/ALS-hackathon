sap.ui.jsview("view.handwriting", {
	...
	createContent : function(oController) {
		...
		var headerLayout = new sap.ui.layout.HorizontalLayout({
			content: [
			          new sap.m.Label("",{"text":"Follow the shape using your finger or stylus"}).addStyleClass("question_title"),
			          new sap.m.Label({"text":"1/3"}).addStyleClass("page_num")
			          ]
		}).addStyleClass('questions_header');

		var h1 = new sap.ui.layout.VerticalLayout('h1', {content: [new sap.ui.core.HTML({
			content:   "<canvas id='houseCanvas'  width='600' height='600' >" +   "</canvas>"})]}
		).addStyleClass("hwCanvas");     

		var html1 = new sap.ui.layout.VerticalLayout('html1', {content: [headerLayout, h1]}).addStyleClass("viz");


		var headerLayout = new sap.ui.layout.HorizontalLayout({
			content: [
			          new sap.m.Label("",{"text":"Follow the shape using your finger or stylus"}).addStyleClass("question_title"),
			          new sap.m.Label({"text":"2/3"}).addStyleClass("page_num")
			          ]
		}).addStyleClass('questions_header');

		var h2 = new sap.ui.layout.VerticalLayout('h2', {content: [new sap.ui.core.HTML({
			content:   "<canvas id='smileyCanvas'  width='600' height='600' >" +   "</canvas>"})]}
		).addStyleClass("hwCanvas");     

		var html2 = new sap.ui.layout.VerticalLayout('html2', {content: [headerLayout, h2]}).addStyleClass("viz");
		var headerLayout = new sap.ui.layout.HorizontalLayout({
			content: [
			          new sap.m.Label("",{"text":"Write down the following sentence:"}).addStyleClass("question_title"),
			          new sap.m.Label({"text":"3/3"}).addStyleClass("page_num")
			          ]
		}).addStyleClass('questions_header');

		var sentence = new sap.ui.layout.HorizontalLayout({
			content: [
			          new sap.m.Label("",{"text":"'Hello John, how are you?'"}).addStyleClass("question_title"),
			          ]
		}).addStyleClass('questions_header');

		var h3 = new sap.ui.layout.VerticalLayout('h3', {content: [new sap.ui.core.HTML({
			content:   "<canvas id='sentenceCanvas'  width='600' height='600' >" +   "</canvas>"})]}
		).addStyleClass("hwCanvas");     

		var html3 = new sap.ui.layout.VerticalLayout('html3', {content: [headerLayout,sentence, h3]}).addStyleClass("viz");
		...
		/************** Buttons **************/		
		this.btnClear = new sap.m.Button({text:"Clear", press:function onClear() {
			var drawing = me.getDrawingByCanvasName(me.currCanvas);
			if (!drawing) return;
			me.clearDrawing(drawing);
		}});

		this.btnFinish = new sap.m.Button({text:"Finish", press:function onFinish() {
			...
			var result = "[{\"writingHouse\":" + houseJsonStr + "}]" + "[{\"writingSmiley\":" + smileyJsonStr + "}]" + "[{\"writingSentence\":" + sentenceJsonStr + "}]"; 
			//TODO: send the result to the server here
			//TODO: remove alert
			alert("This JSON string should be sent to the server:" + result);
		...
		this.btnNext = new sap.m.Button({text:"Next", press:function nextTo() {
			switch(me.currCanvas){
			case "houseCanvas" :
				if (!me.smileyDrawing)
					setTimeout(function(){me.smileyDrawing = new RecordableDrawing("smileyCanvas", "images/smiley.png");me.smileyDrawing.startRecording();},0);
				me.currCanvas = "smileyCanvas";
				nav.to("html2");
				break;
			case "smileyCanvas" :
				if (!me.sentenceDrawing)
					setTimeout(function(){me.sentenceDrawing = new RecordableDrawing("sentenceCanvas");me.sentenceDrawing.startRecording();},0);
		...
		this.btnBack = new sap.m.Button({text:"Back", press:function backTo(page, param1, param2) {
		...
		var flt = new sap.ui.layout.HorizontalLayout('footerLt', {content: [this.btnBack, this.btnNext, this.btnClear,  this.btnFinish]});
		...
		return new sap.ui.layout.VerticalLayout("handwriting_layout", {content: [header, nav, footer], width: '100%'});
	},
